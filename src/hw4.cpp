...
long long net_overflow_score(const Path& path) {
    ...
    for (size_t i = 1; i < path.size(); ++i) {
        ...
        if (e.demand > e.cap) s += (e.demand - e.cap);
    }
    return s;
}
...
vector<int> pick_topK_nets_by_score(const vector<Path>& paths, int K) {
    ...
    for (int i = 0; i < (int)paths.size(); ++i) {
        ...
        if (sc > 0) score_id.push_back({sc, i});
    }
...
// ==========================
// Fast A* with global buffers + stamps
// ==========================
                              ...
                              bool debug_print_g_h_f)
{
    ...
    while (!pq.empty()) {
        ...
        if (closedStamp[u] == CUR_STAMP) continue;
        ...
        for (int d = 0; d < 4; ++d) {
            ...
            int v = vid(nx, ny);
            if (closedStamp[v] == CUR_STAMP) continue;
...
// ==========================
void polish_wirelength(vector<Path>& cur_paths, int rounds = 2) {
    for (int r = 0; r < rounds; ++r) {
        ...
        for (int id : order) {
            ...
            // 純線長 + 硬容量：ALPHA=0,BETA=0, forbidOverflow=true
...
// ==========================
// Main routing (fast selective RNR + WL polish)
// ==========================
void Astar_search_fast() {
    const int INITIAL_ONLY = 20;
    ...
    // 0) initial routing (one-time)
    reset_all_demands();

    for (int t = 0; t < INITIAL_ONLY; ++t) {
        if (t > 0) reset_all_demands();
        ...
        for (int i = 0; i < (int)netlist.size(); ++i) {
            ...
            if (path.empty()) {
                ...
                int x = sx, y = sy;
                path.push_back({x,y});
                while (x != tx) { x += (tx > x ? 1 : -1); path.push_back({x,y}); }
                while (y != ty) { y += (ty > y ? 1 : -1); path.push_back({x,y}); }
            }
    ...
    // 1) repair loop: selective rip-up & reroute
    for (int iter = 0; iter < REPAIR_ITERS; ++iter) {
        ...
        if (rip_ids.empty() && !cur_paths.empty()) {
            ...
            for (int i = 1; i < (int)cur_paths.size(); ++i)
                if (cur_paths[i].size() > cur_paths[worst].size()) worst = i;
        ...
        for (int id : rip_ids) {
            ...
            if (newp.empty()) {
                int x = sx, y = sy;
                newp.push_back({x,y});
                while (x != tx) { x += (tx > x ? 1 : -1); newp.push_back({x,y}); }
                while (y != ty) { y += (ty > y ? 1 : -1); newp.push_back({x,y}); }
            }
    ...
    // 3) WL Polish on best solution (overflow hard constraint)
    all_paths = best_paths;
    polish_wirelength(all_paths, /*rounds=*/2);
...
// ==========================
// Output formatting (same as yours)
// ==========================
